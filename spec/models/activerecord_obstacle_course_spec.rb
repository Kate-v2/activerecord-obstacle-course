...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    order_ids = OrderItem.where(item_id: item_8.id).pluck(:order_id)
    user_ids = Order.where(id: order_ids).pluck(:user_id)
    users = User.where(id: user_ids).order(:name).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    #  Solution goes here
    orders = Order.joins(:order_items).where(user_id: user_2.id).where('item_id = ?', item_4.id)
    # -----------------------------------------------------------
